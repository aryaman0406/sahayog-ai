export const te = {
  brand: "సహయోగ్ AI",
  navHome: "హోమ్",
  navDashboard: "డ్యాష్‌బోర్డ్",
  navSaved: "బుక్‌మార్క్‌లు",
  navAnalytics: "విశ్లేషణలు",
  navLogin: "లాగిన్",
  navRegister: "రిజిస్టర్",
  navLogout: "లాగౌట్",
  btnBack: "వెనుకకు",
  btnSave: "సేవ్ చేయి",
  btnSaved: "సేవ్ చేయబడింది",
  btnRemove: "తొలగించు",
  btnShare: "షేర్ చేయి",
  btnApply: "దరఖాస్తు చేసుకోండి",

  // Landing / Hero
  heroEyebrow: "AI ద్వారా పౌరుల సాధికారత",
  heroTitle: "సహయోగ్ AI",
  heroSubtitle: "మీరు అర్హత పొందే భారత ప్రభుత్వ సంక్షేమ పథకాలను కనుగొనండి, సరళమైన భాషలో ప్రయోజనాలను అర్థం చేసుకోండి మరియు ఎలా దరఖాస్తు చేసుకోవాలో దశల వారీ మార్గదర్శకత్వాన్ని పొందండి.",
  heroCTA: "అర్హత కలిగిన పథకాలను కనుగొనండి",
  statSchemes: "జాబితా చేయబడిన పథకాలు",
  statTime: "ధృవీకరణ సమయం",
  statInstant: "తక్షణమే",
  statUsers: "క్రియాశీల వినియోగదారులు",

  // AuthForm
  authLoginTitle: "స్వాగతం",
  authLoginSubtitle: "మీ సరిపోలిన పథకాలను తనిఖీ చేయడానికి మరియు బుక్‌మార్క్‌లను ట్రాక్ చేయడానికి లాగిన్ చేయండి.",
  authRegisterTitle: "ఖాతాను సృష్టించండి",
  authRegisterSubtitle: "మీ ప్రొఫైల్‌ను సేవ్ చేయడానికి మరియు అర్హత కలిగిన సంక్షేమ పథకాలను కనుగొనడానికి రిజిస్టర్ చేయండి.",
  authNameLabel: "పూర్తి పేరు",
  authEmailLabel: "ఈమెయిల్ చిరునామా",
  authPasswordLabel: "పాస్‌వర్డ్",
  authLoginBtn: "లాగిన్",
  authRegisterBtn: "ఖాతాను సృష్టించండి",
  authNoAccount: "ఖాతా లేదా?",
  authHaveAccount: "ఇప్పటికే ఖాతా ఉందా?",

  // SchemeResults
  resultsTitle: "మీ అర్హత కలిగిన పథకాలు",
  resultsSubtitle: "మీ ప్రొఫైల్ ఆధారంగా, మీరు అర్హత పొందే పథకాలు ఇక్కడ ఉన్నాయి.",
  noMatches: "మీ ప్రొఫైల్ వివరాలతో ఏ పథకమూ సరిపోలడం లేదు. పైన ఉన్న శోధన పట్టీలో వెతకడానికి ప్రయత్నించండి.",
  searchPlaceholder: "పేరు, వర్గం లేదా వివరణ ద్వారా పథకాలను వెతకండి...",
  matchScore: "సరిపోలిక స్కోర్",
  schemeDetailTitle: "పథకం సమాచారం",
  schemeBenefits: "పథకం ప్రయోజనాలు",
  schemeEligibility: "అర్హత ప్రమాణాలు",
  schemeDocuments: "కావలసిన పత్రాలు",
  schemeApply: "దరఖాస్తు విధానం",
  copiedLink: "లింక్ కాపీ చేయబడింది!",
  viewDetail: "వివరాలు చూడండి",
  loadingScheme: "పథకం వివరాలు లోడ్ అవుతున్నాయి...",
  translatingTitle: "శీర్షిక అనువదింపబడుతోంది...",
  checkEligibility: "నా అర్హత తనిఖీ చేయండి →",
  similarSchemes: "సరిపోలిన పథకాలు",
  profileFormTitle: "అర్హత ప్రశ్నావళి",
  profileFormSubtitle: "సరిపోలే సంక్షేమ పథకాలను స్కాన్ చేయడానికి మీ గురించి కొన్ని వివరాలను తెలియజేయండి.",
  profileAge: "వయస్సు",
  profileIncome: "సంవత్సర ఆదాయం (₹)",
  profileOccupation: "ఉద్యోగం/వృత్తి",
  profileArea: "ప్రాంతం",
  profileGender: "లింగం",
  profileLanguage: "ప్రాధాన్యత కలిగిన భాష",
  profileSubmit: "సరిపోలే పథకాలను చూపించు",
  profileUpdating: "ప్రొఫైల్ అప్‌డేట్ అవుతోంది...",
  rural: "గ్రామీణ",
  urban: "పట్టణ",
  male: "పురుషుడు",
  female: "స్త్రీ",
  other: "ఇతర",
  farmer: "రైతు",
  student: "విద్యార్థి",
  "self-employed": "స్వయం ఉపాధి",
  unemployed: "నిరుద్యోగి",
  salaried: "ఉద్యోగి (జీతం పొందేవాడు)",
  any: "ఏదైనా",
  savedTitle: "నా బుక్‌మార్క్ పథకాలు",
  savedSubtitle: "త్వరిత సూచన కోసం మీరు ఈ పథకాలను సేవ్ చేసుకున్నారు.",
  noSaved: "మీరు ఇంకా ఏ పథకాన్ని బుక్‌మార్క్ చేయలేదు.",
  chatTitle: "సహయోగ్ AI అసిస్టెంట్‌ని అడగండి",
  chatMeta: "తెలుగులో నిజ-సమయ సమాధానాలు",
  chatVoiceOn: "వాయిస్ సమాధానాలు: ఆన్",
  chatVoiceOff: "వాయిస్ సమాధానాలు: ఆఫ్",
  chatPlaceholder: "పత్రాలు, ప్రయోజనాలు లేదా దరఖాస్తు గురించి అడగండి...",
  chatSend: "పంపు",
  chatListening: "వింటున్నాము...",
  chatMicBtn: "మాట్లాడటానికి నొక్కి పట్టుకోండి",
  chatMicStop: "మైక్ ఆపివేయి",
  chatConnecting: "చాట్ సర్వర్‌కు సంపర్కిస్తున్నాము...",
  chatAskAny: "మీ ఇష్టమైన భాషలో ప్రశ్నలు అడగండి.",
  chatDisconnected: "చాట్ సర్వర్ సంపర్కం తెగిపోయింది. మళ్ళీ సంపర్కిస్తున్నాము...",
  pdfGenerating: "గైడ్ సిద్ధం అవుతుంది...",
  pdfDownload: "📥 PDF గైడ్ డౌన్‌లోడ్ చేయండి",

  // Analytics
  analyticsTitle: "నా విశ్లేషణలు & అంతర్దృష్టులు",
  analyticsSubtitle: "పథకాల లభ్యత మరియు సరిపోలికల ఉన్నత స్థాయి సారాంశం.",
  analyticsTotalMatched: "మొత్తం సరిపోలిన పథకాలు",
  analyticsTopCategory: "అత్యుత్తమ అర్హత కలిగిన వర్గం",
  analyticsIncomeStatus: "ఆదాయ స్థితి",
  analyticsIncomeBelow: "దారిద్య్ర రేఖకు దిగువన అర్హులు (BPL)",
  analyticsIncomeAbove: "దారిద్య్ర రేఖకు ఎగువన అర్హులు (APL)",
  analyticsChartTitle: "వర్గం వారీగా అర్హత కలిగిన పథకాలు",
  analyticsEmpty: "మీ విశ్లేషణలు చూడటానికి డాష్బోర్డ్లో మీ ప్రొఫైల్ని పూర్తి చేయండి.",
  catAgriculture: "వ్యవసాయం 🌾",
  catHealthcare: "ఆరోగ్య సేవ 🏥",
  catHousing: "గృహనిర్మాణం 🏠",
  catEducation: "విద్య 🎓",
  catEmployment: "ఉపాధి మరియు నైపుణ్యాలు 💼",
  catSocialWelfare: "సామాజిక సంక్షేమం మరియు సాధికారత 🤝",
  catBusiness: "వ్యాపారం మరియు ఉపాధి 🏢",
  catBanking: "బ్యాంకింగ్ మరియు ఆర్థిక సేవలు 🏦",
  catWomenChild: "మహిళ మరియు శిశు సంక్షేమం 👩‍👧",
  catUtility: "సౌకర్యాలు మరియు పారిశుధ్యం 🧹",
  catTransport: "రవాణా మరియు మౌలిక సదుపాయాలు 🚜",
  catTravel: "ప్రయాణం మరియు పర్యాటకం 🧳",
  catSports: "క్రీడలు మరియు సంస్కృతి 🏆",
  catScience: "సైన్స్ మరియు సాంకేతికత 💻",
  catSafety: "ప్రజా రక్షణ మరియు చట్టం ⚖️",
  catHousehold: "గృహ సంబంధిత 🔥",
  catPension: "పెన్షన్ 👵",
  catGeneral: "సాధారణ 📜",
  chatFloatingBtn: "సహయోగ్ AI అడగండి",
  chatFloatingTooltip: "సహాయం కావాలా? AI సహాయకుడితో చాట్ చేయడానికి క్లిక్ చేయండి",
};
export const gu = {
  brand: "સહયોગ AI",
  navHome: "હોમ",
  navDashboard: "ડેશબોર્ડ",
  navSaved: "બુકમાર્ક",
  navAnalytics: "વિશ્લેષણ",
  navLogin: "લોગ ઇન",
  navRegister: "રજીસ્ટર",
  navLogout: "લોગ આઉટ",
  btnBack: "પાછા",
  btnSave: "સાચવો",
  btnSaved: "સાચવેલ",
  btnRemove: "દૂર કરો",
  btnShare: "શેર કરો",
  btnApply: "અરજી કરો",

  // Landing / Hero
  heroEyebrow: "AI દ્વારા નાગરિકોનું સશક્તિકરણ",
  heroTitle: "સહયોગ AI",
  heroSubtitle: "તમે જે ભારતીય સરકારી કલ્યાણકારી યોજનાઓ માટે પાત્ર છો તે શોધો, સરળ ભાષામાં તેના ફાયદા સમજો અને અરજી કેવી રીતે કરવી તેનું સ્ટેપ-બાય-સ્ટેપ માર્ગદર્શન મેળવો.",
  heroCTA: "પાત્ર યોજનાઓ શોધો",
  statSchemes: "સૂચિબદ્ધ યોજનાઓ",
  statTime: "ચકાસણી સમય",
  statInstant: "ત્વરિત",
  statUsers: "સક્રિય વપરાશકર્તાઓ",

  // AuthForm
  authLoginTitle: "ફરી સ્વાગત છે",
  authLoginSubtitle: "તમારી મેળ ખાતી યોજનાઓ તપાસવા અને બુકમાર્ક્સ ટ્રૅક કરવા લોગ ઇન કરો.",
  authRegisterTitle: "ખાતું બનાવો",
  authRegisterSubtitle: "તમારી પ્રોફાઇલ સાચવવા અને પાત્ર કલ્યાણકારી યોજનાઓ શોધવા માટે રજીસ્ટર કરો.",
  authNameLabel: "પૂરું નામ",
  authEmailLabel: "ઈમેલ સરનામું",
  authPasswordLabel: "પાસવર્ડ",
  authLoginBtn: "લોગ ઇન",
  authRegisterBtn: "ખાતું બનાવો",
  authNoAccount: "ખાતું નથી?",
  authHaveAccount: "પહેલેથી જ ખાતું છે?",

  // SchemeResults
  resultsTitle: "તમારી પાત્ર યોજનાઓ",
  resultsSubtitle: "તમારી પ્રોફાઇલના આધારે, તમે આ યોજનાઓ માટે પાત્ર છો.",
  noMatches: "તમારી પ્રોફાઇલ વિગતો સાથે કોઈ યોજના મેળ ખાતી નથી. ઉપરના સર્ચ બારમાં શોધવાનો પ્રયાસ કરો.",
  searchPlaceholder: "નામ, શ્રેણી અથવા વર્ણન દ્વારા યોજનાઓ શોધો...",
  matchScore: "મેળ ખાતો સ્કોર",
  schemeDetailTitle: "યોજનાની માહિતી",
  schemeBenefits: "યોજનાના લાભો",
  schemeEligibility: "પાત્રતાના માપદંડ",
  schemeDocuments: "જરૂરી દસ્તાવેજો",
  schemeApply: "કેવી રીતે અરજી કરવી",
  copiedLink: "લિંક કોપી થઈ ગઈ!",
  viewDetail: "વિગત જુઓ",
  loadingScheme: "યોજનાની વિગત લોડ થઈ રહી છે...",
  translatingTitle: "શીર્ષક અનુવાદ થઈ રહ્યું છે...",
  checkEligibility: "મારી પાત્રતા તપાસો →",
  similarSchemes: "સમાન યોજનાઓ",
  profileFormTitle: "પાત્રતા પ્રશ્નાવલી",
  profileFormSubtitle: "યોગ્ય કલ્યાણકારી યોજનાઓ સ્કેન કરવા માટે તમારા વિશે થોડી વિગતો જણાવો.",
  profileAge: "ઉંમર",
  profileIncome: "વાર્ષિક કૌટુંબિક આવક (₹)",
  profileOccupation: "વ્યવસાય",
  profileArea: "વિસ્તાર",
  profileGender: "જાતિ",
  profileLanguage: "પસંદગીની ભાષા",
  profileSubmit: "યોગ્ય યોજનાઓ બતાવો",
  profileUpdating: "પ્રોફાઇલ અપડેટ થઈ રહી છે...",
  rural: "ગ્રામીણ",
  urban: "શહેરી",
  male: "પુરુષ",
  female: "મહિલા",
  other: "અન્ય",
  farmer: "ખેડૂત",
  student: "વિદ્યાર્થી",
  "self-employed": "સ્વરોજગાર",
  unemployed: "બેરોજગાર",
  salaried: "પગારદાર",
  any: "કોઈપણ",
  savedTitle: "મારી સેવ કરેલી યોજનાઓ",
  savedSubtitle: "ત્વરિત સંદર્ભ માટે તમે આ યોજનાઓ સાચવી છે.",
  noSaved: "તમે હજી સુધી કોઈ યોજના બુકમાર્ક કરી નથી.",
  chatTitle: "સહયોગ AI મદદનીશને પૂછો",
  chatMeta: "ગુજરાતીમાં રીઅલ-ટાઇમ જવાબો",
  chatVoiceOn: "અવાજ જવાબો: ચાલુ",
  chatVoiceOff: "અવાજ જવાબો: બંધ",
  chatPlaceholder: "દસ્તાવેજો, લાભો અથવા અરજી કરવા વિશે પૂછો...",
  chatSend: "મોકલો",
  chatListening: "સાંભળી રહ્યા છીએ...",
  chatMicBtn: "બોલવા માટે દબાવી રાખો",
  chatMicStop: "માઈક બંધ કરો",
  chatConnecting: "ચેટ સર્વર સાથે જોડાઈ રહ્યું છે...",
  chatAskAny: "તમારી પસંદગીની ભાષામાં પ્રશ્ન પૂછો.",
  chatDisconnected: "ચેટ સર્વર ડિસ્કનેક્ટ થઈ ગયું. ફરી જોડાઈ રહ્યું છે...",
  pdfGenerating: "માર્ગદર્શિકા તૈયાર થઈ રહી છે...",
  pdfDownload: "📥 PDF માર્ગદર્શિકા ડાઉનલોડ કરો",

  // Analytics
  analyticsTitle: "મારા વિશ્લેષણ અને આંતરદૃષ્ટિ",
  analyticsSubtitle: "યોજનાઓની ઉપલબ્ધતા અને મેળ ખાતા આંકડાનો ઉચ્ચ સ્તરીય સારાંશ.",
  analyticsTotalMatched: "કુલ મેળ ખાતી યોજનાઓ",
  analyticsTopCategory: "ટોચની પાત્ર શ્રેણી",
  analyticsIncomeStatus: "આવકની સ્થિતિ",
  analyticsIncomeBelow: "ગરીબી રેખા નીચે પાત્ર (BPL)",
  analyticsIncomeAbove: "ગરીબી રેખા ઉપર પાત્ર (APL)",
  analyticsChartTitle: "શ્રેણી અનુસાર પાત્ર યોજનાઓ",
  analyticsEmpty: "તમારું વિશ્લેષણ જોવા ડેશબોર્ડ પર તમારી પ્રોફાઇલ પૂર્ણ કરો.",
  catAgriculture: "ખેતી 🌾",
  catHealthcare: "આરોગ્ય સેવા 🏥",
  catHousing: "ગૃહ નિર્માણ 🏠",
  catEducation: "શિક્ષણ 🎓",
  catEmployment: "રોજગાર અને કૌશલ્ય 💼",
  catSocialWelfare: "સામાજિક કલ્યાણ અને સશક્તિકરણ 🤝",
  catBusiness: "વ્યાપાર અને ઉદ્યોગસાહસિકતા 🏢",
  catBanking: "બેંકિંગ અને નાણાકીય સેવાઓ 🏦",
  catWomenChild: "મહિલા અને બાળ વિકાસ 👩‍👧",
  catUtility: "ઉપયોગિતા અને સ્વચ્છતા 🧹",
  catTransport: "પરિવહન અને માળખાકીય સુવિધાઓ 🚜",
  catTravel: "પ્રવાસ અને પ્રવાસન 🧳",
  catSports: "રમતગમત અને સંસ્કૃતિ 🏆",
  catScience: "વિજ્ઞાન અને ટેકનોલોજી 💻",
  catSafety: "જાહેર સુરક્ષા અને કાયદો ⚖️",
  catHousehold: "ઘરેલું 🔥",
  catPension: "પેન્શન 👵",
  catGeneral: "સામાન્ય 📜",
  chatFloatingBtn: "સહયોગ AI ને પૂછો",
  chatFloatingTooltip: "મદદ જોઈએ છે? AI સહાયક સાથે ચેટ કરવા ક્લિક કરો",
};
export const kn = {
  brand: "ಸಹಯೋಗ್ AI",
  navHome: "ಹೋಮ್",
  navDashboard: "ಡ್ಯಾಶ್‌ಬೋರ್ಡ್",
  navSaved: "ಬುಕ್‌ಮಾರ್ಕ್‌ಗಳು",
  navAnalytics: "ವಿಶ್ಲೇಷಣೆ",
  navLogin: "ಲಾಗಿನ್",
  navRegister: "ನೋಂದಣಿ",
  navLogout: "ಲಾಗ್ ಔಟ್",
  btnBack: "ಹಿಂದಕ್ಕೆ",
  btnSave: "ಉಳಿಸಿ",
  btnSaved: "ಉಳಿಸಲಾಗಿದೆ",
  btnRemove: "ತೆಗೆದುಹಾಕಿ",
  btnShare: "ಹಂಚಿಕೊಳ್ಳಿ",
  btnApply: "ಅರ್ಜಿ ಸಲ್ಲಿಸಿ",

  // Landing / Hero
  heroEyebrow: "AI ಮೂಲಕ ನಾಗರಿಕರ ಸಬಲೀಕರಣ",
  heroTitle: "ಸಹಯೋಗ್ AI",
  heroSubtitle: "ನೀವು ಅರ್ಹತೆ ಪಡೆಯುವ ಭಾರತ ಸರ್ಕಾರದ ಕಲ್ಯಾಣ ಯೋಜನೆಗಳನ್ನು ಅನ್ವೇಷಿಸಿ, ಸರಳ ಭಾಷೆಯಲ್ಲಿ ಪ್ರಯೋಜನಗಳನ್ನು ಅರ್ಥಮಾಡಿಕೊಳ್ಳಿ ಮತ್ತು ಹೇಗೆ ಅರ್ಜಿ ಸಲ್ಲಿಸಬೇಕು ಎಂಬುದರ ಕುರಿತು ಹಂತ-ಹಂತದ ಮಾರ್ಗದರ್ಶನ ಪಡೆಯಿರಿ.",
  heroCTA: "ಅರ್ಹ ಯೋಜನೆಗಳನ್ನು ಹುಡುಕಿ",
  statSchemes: "ಪಟ್ಟಿ ಮಾಡಲಾದ ಯೋಜನೆಗಳು",
  statTime: "ಪರಿಶೀಲನಾ ಸಮಯ",
  statInstant: "ತತ್‌ಕ್ಷಣ",
  statUsers: "ಸಕ್ರಿಯ ಬಳಕೆದಾರರು",

  // AuthForm
  authLoginTitle: "ಸ್ವಾಗತ",
  authLoginSubtitle: "ನಿಮ್ಮ ಹೊಂದಾಣಿಕೆಯ ಯೋಜನೆಗಳನ್ನು ಪರಿಶೀಲಿಸಲು ಮತ್ತು ಬುಕ್‌ಮಾರ್ಕ್‌ಗಳನ್ನು ಟ್ರ್ಯಾಕ್ ಮಾಡಲು ಲಾಗಿನ್ ಮಾಡಿ.",
  authRegisterTitle: "ಖಾತೆಯನ್ನು ರಚಿಸಿ",
  authRegisterSubtitle: "ನಿಮ್ಮ ಪ್ರೊಫೈಲ್ ಉಳಿಸಲು ಮತ್ತು ಅರ್ಹ ಕಲ್ಯಾಣ ಯೋಜನೆಗಳನ್ನು ಅನ್ವೇಷಿಸಲು ನೋಂದಾಯಿಸಿ.",
  authNameLabel: "ಪ್ರತಿ ಹೆಸರು",
  authEmailLabel: "ಇಮೇಲ್ ವಿಳಾಸ",
  authPasswordLabel: "ಪಾಸ್‌ವರ್ಡ್",
  authLoginBtn: "ಲಾಗಿನ್",
  authRegisterBtn: "ಖಾತೆಯನ್ನು ರಚಿಸಿ",
  authNoAccount: "ಖಾತೆ ಇಲ್ಲವೇ?",
  authHaveAccount: "ಈಗಾಗಲೇ ಖಾತೆ ಇದೆಯೇ?",

  // SchemeResults
  resultsTitle: "ನಿಮ್ಮ ಅರ್ಹ ಯೋಜನೆಗಳು",
  resultsSubtitle: "ನಿಮ್ಮ ಪ್ರೊಫೈಲ್ ಆಧಾರದ ಮೇಲೆ, ನೀವು ಅರ್ಹತೆ ಪಡೆಯುವ ಯೋಜನೆಗಳು ಇಲ್ಲಿವೆ.",
  noMatches: "ನಿಮ್ಮ ಪ್ರೊಫೈಲ್ ವಿವರಗಳಿಗೆ ಯಾವುದೇ ಯೋಜನೆ ಹೊಂದಿಕೆಯಾಗುವುದಿಲ್ಲ. ಮೇಲಿನ ಹುಡುಕಾಟ ಪಟ್ಟಿಯಲ್ಲಿ ಹುಡುಕಲು ಪ್ರಯತ್ನಿಸಿ.",
  searchPlaceholder: "ಹೆಸರು, ವರ್ಗ ಅಥವಾ ವಿವರಣೆಯ ಮೂಲಕ ಯೋಜನೆಗಳನ್ನು ಹುಡುಕಿ...",
  matchScore: "ಹೊಂದಾಣಿಕೆಯ ಸ್ಕೋರ್",
  schemeDetailTitle: "ಯೋಜನೆಯ ಮಾಹಿತಿ",
  schemeBenefits: "ಯೋಜನೆಯ ಪ್ರಯೋಜನಗಳು",
  schemeEligibility: "ಅರ್ಹತೆಯ ಮಾನದಂಡ",
  schemeDocuments: "ಅಗತ್ಯ ದಾಖಲೆಗಳು",
  schemeApply: "ಅರ್ಜಿ ಸಲ್ಲಿಸುವುದು ಹೇಗೆ",
  copiedLink: "ಲಿಂಕ್ ಕಾಪಿ ಮಾಡಲಾಗಿದೆ!",
  viewDetail: "ವಿವರಗಳನ್ನು ನೋಡಿ",
  loadingScheme: "ಯೋಜನೆಯ ವಿವರಗಳನ್ನು ಲೋಡ್ ಮಾಡಲಾಗುತ್ತಿದೆ...",
  translatingTitle: "ಶೀರ್ಷಿಕೆ ಅನುವಾದಿಸಲಾಗುತ್ತಿದೆ...",
  checkEligibility: "ನನ್ನ ಅರ್ಹತೆ ಪರಿಶೀಲಿಸಿ →",
  similarSchemes: "ಹೋಂದಾಣಿಕೆಯ ಯೋಜನೆಗಳು",
  profileFormTitle: "ಅರ್ಹತೆಯ ಪ್ರಶ್ನಾವಳಿ",
  profileFormSubtitle: "ಹೊಂದಾಣಿಕೆಯ ಕಲ್ಯಾಣ ಯೋಜನೆಗಳನ್ನು ಸ್ಕ್ಯಾನ್ ಮಾಡಲು ನಿಮ್ಮ ಬಗ್ಗೆ ಕೆಲವು ವಿವರಗಳನ್ನು ತಿಳಿಸಿ.",
  profileAge: "ವಯಸ್ಸು",
  profileIncome: "ವಾರ್ಷಿಕ ಕೌಟುಂಬಿಕ ಆದಾಯ (₹)",
  profileOccupation: "ಉದ್ಯೋಗ",
  profileArea: "ಪ್ರದೇಶ",
  profileGender: "ಲಿಂಗ",
  profileLanguage: "ಆದ್ಯತೆಯ ಭಾಷೆ",
  profileSubmit: "ಹೊಂದಾಣಿಕೆಯ ಯೋಜನೆಗಳನ್ನು ತೋರಿಸಿ",
  profileUpdating: "ಪ್ರೊಫೈಲ್ ನವೀಕರಿಸಲಾಗುತ್ತಿದೆ...",
  rural: "ಗ್ರಾಮೀಣ",
  urban: "ನಗರ",
  male: "ಪುರುಷ",
  female: "ಮಹಿಳೆ",
  other: "ಇತರ",
  farmer: "ರೈತ",
  student: "ವಿದ್ಯಾರ್ಥಿ",
  "self-employed": "ಸ್ವಯಂ ಉದ್ಯೋಗಿ",
  unemployed: "ನಿರುದ್ಯೋಗಿ",
  salaried: "ವೇತನದಾರ",
  any: "ಯಾವುದಾದರೂ",
  savedTitle: "ನನ್ನ ಬುಕ್‌ಮಾರ್ಕ್ ಯೋಜನೆಗಳು",
  savedSubtitle: "ತ್ವರಿತ ಉಲ್ಲೇಖಕ್ಕಾಗಿ ನೀವು ಈ ಯೋಜನೆಗಳನ್ನು ಉಳಿಸಿದ್ದೀರಿ.",
  noSaved: "ನೀವು ಇನ್ನೂ ಯಾವುದೇ ಯೋಜನೆಯನ್ನು ಬುಕ್‌ಮಾರ್ಕ್ ಮಾಡಿಲ್ಲ.",
  chatTitle: "ಸಹಯೋಗ್ AI ಸಹಾಯಕನನ್ನು ಕೇಳಿ",
  chatMeta: "ಕನ್ನಡದಲ್ಲಿ ನೈಜ-ಸಮಯದ ಪ್ರತಿಕ್ರಿಯೆಗಳು",
  chatVoiceOn: "ಧ್ವನಿ ಪ್ರತಿಕ್ರಿಯೆಗಳು: ಆನ್",
  chatVoiceOff: "ಧ್ವನಿ ಪ್ರತಿಕ್ರಿಯೆಗಳು: ಆಫ್",
  chatPlaceholder: "ದಾಖಲೆಗಳು, ಪ್ರಯೋಜನಗಳು ಅಥವಾ ಅರ್ಜಿ ಸಲ್ಲಿಸುವ ಬಗ್ಗೆ ಕೇಳಿ...",
  chatSend: "ಕಳುಹಿಸಿ",
  chatListening: "ಕೇಳಿಸಿಕೊಳ್ಳಲಾಗುತ್ತಿದೆ...",
  chatMicBtn: "ಮಾತನಾಡಲು ಒತ್ತಿ ಹಿಡಿಯಿರಿ",
  chatMicStop: "ಮೈಕ್ ನಿಲ್ಲಿಸಿ",
  chatConnecting: "ಚಾಟ್ ಸರ್ವರ್ಗೆ ಸಂಪರ್ಕಿಸಲಾಗುತ್ತಿದೆ...",
  chatAskAny: "ನಿಮ್ಮ ಆದ್ಯತೆಯ ಭಾಷೆಯಲ್ಲಿ ಪ್ರಶ್ನಗಳನ್ನು ಕೇಳಿ.",
  chatDisconnected: "ಚಾಟ್ ಸರ್ವರ್ ಸಂಪರ್ಕ ತಂದಿದೆ. ಮತ್ತೆ ಸಂಪರ್ಕಿಸುತ್ತಿದೆ...",
  pdfGenerating: "ಮಾರ್ಗದರ್ಶಿ ಸಿದ್ಧಪಡುತ್ತಿದೆ...",
  pdfDownload: "📥 PDF ಮಾರ್ಗದರ್ಶಿ ಡೌನ್ಲೋಡ್ ಮಾಡಿ",

  // Analytics
  analyticsTitle: "ನನ್ನ ಒಳನೋಟಗಳು ಮತ್ತು ವಿಲೇಷಣೆ",
  analyticsSubtitle: "ಯೋಜನೆಗಳ ಲಭ್ಯತೆ ಮತ್ತು ಹೊಂದಾಣಿಕೆಗಳ ಉನ್ನತ ಮಟ್ಟದ ಸಾರಾಂಶ.",
  analyticsTotalMatched: "ಒಟ್ಟು ಹೊಂದಾಣಿಕೆಯಾದ ಯೋಜನೆಗಳು",
  analyticsTopCategory: "ಉನ್ನತ ಅರ್ಹತಾ ವರ್ಗ",
  analyticsIncomeStatus: "ಆದಾಯ ಸ್ಥಿತಿ",
  analyticsIncomeBelow: "ಬಡತನ ರೇಖೆಗಿಂತ ಕೆಳಗಿನ ಅರ್ಹರು (BPL)",
  analyticsIncomeAbove: "ಬಡತನ ರೇಖೆಗಿಂತ ಮೇಲಿನ ಅರ್ಹರು (APL)",
  analyticsChartTitle: "ವರ್ಗದ ಪ್ರಕಾರ ಅರ್ಹ ಯೋಜನೆಗಳು",
  analyticsEmpty: "ನಿಮ್ಮ ವಿಶ್ಲೇಷಣೆ ವೀಕ್ಷಿಸಲು ಡ್ಯಾಶ್ಬೋರ್ಡ್ನಲ್ಲಿ ನಿಮ್ಮ ಪ್ರೊಫೈಲ್ ಪೂರ್ಣಗೊಳಿಸಿ.",
  catAgriculture: "ಕೃಷಿ 🌾",
  catHealthcare: "ಆರೋಗ್ಯ ಸೇವೆ 🏥",
  catHousing: "ವಸತಿ 🏠",
  catEducation: "ಶಿಕ್ಷಣ 🎓",
  catEmployment: "ಉದ್ಯೋಗ ಮತ್ತು ಕೌಶಲ್ಯ 💼",
  catSocialWelfare: "ಸಾಮಾಜಿಕ ಕಲ್ಯಾಣ ಮತ್ತು ಸಬಲೀಕರಣ 🤝",
  catBusiness: "ವ್ಯಾಪಾರ ಮತ್ತು ಉದ್ಯಮಶೀಲತೆ 🏢",
  catBanking: "ಬ್ಯಾಂಕಿಂಗ್ ಮತ್ತು ಹಣಕಾಸು ಸೇವೆಗಳು 🏦",
  catWomenChild: "ಮಹಿಳೆ ಮತ್ತು ಮಕ್ಕಳ ಅಭಿವೃದ್ಧಿ 👩‍👧",
  catUtility: "ಸೌಲಭ್ಯ ಮತ್ತು ನೈರ್ಮಲ್ಯ 🧹",
  catTransport: "ಸಾರಿಗೆ ಮತ್ತು ಮೂಲಸೌಕರ್ಯ 🚜",
  catTravel: "ಪ್ರವಾಸ ಮತ್ತು ಪ್ರವಾಸೋದ್ಯಮ 🧳",
  catSports: "ಕ್ರೀಡೆ ಮತ್ತು ಸಂಸ್ಕೃತಿ 🏆",
  catScience: "ವಿಜ್ಞಾನ ಮತ್ತು ತಂತ್ರಜ್ಞಾನ 💻",
  catSafety: "ಸಾರ್ವಜನಿಕ ಸುರಕ್ಷತೆ ಮತ್ತು ಕಾನೂನು ⚖️",
  catHousehold: "ಮನೆಗೆ ಸಂಬಂಧಿಸಿದ 🔥",
  catPension: "ಪಿಂಚಣಿ 👵",
  catGeneral: "ಸಾಮಾನ್ಯ 📜",
  chatFloatingBtn: "ಸಹಯೋಗ್ AI ಕೇಳಿ",
  chatFloatingTooltip: "ಸಹಾಯ ಬೇಕೇ? AI ಸಹಾಯಕನೊಂದಿಗೆ ಚಾಟ್ ಮಾಡಲು ಕ್ಲಿಕ್ ಮಾಡಿ",
};
